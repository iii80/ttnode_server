...
端口号信息如下，如需使用端口映射，请在路由器中映射下列端口；若下面显示空白，可在脚本执行完后
一分钟以后执行命令/root/tt_info.sh再次获取端口号，若无需求可忽略
...
甜糖服务的UID如下，若无法在甜糖app中搜索到设备，可在甜糖app中扫码绑定；此后执行
命令/root/tt_info.sh即可获得UID及二维码信息
